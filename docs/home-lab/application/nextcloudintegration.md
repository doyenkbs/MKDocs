...
This guide describes how to integrate <span class="prod-app">Nextcloud</span> with <span class="prod-idp">Authentik</span> for Single Sign-On (SSO) and centralized identity managemen using the OpenID Connect (OIDC) protocol. This setup enables secure authentication for users, eliminating local Nextcloud accounts and centralizing identity management with Authentik.

---

!!! info "**Prerequisites**"

    - A running <span class="prod-app">Nextcloud</span> instance: e.g. `https://nextcloud.example.com`
    - A running <span class="prod-idp">Authentik</span> instance: e.g. `https://authentik.example.com`
    - Admin access to both <span class="prod-idp">Authentik</span> and <span class="prod-app">Nextcloud</span>
...
- Log into **<span class="prod-app">Nextcloud</span>** (`https://nextcloud.example.com`) as an admin.
...
1. Log into <span class="prod-idp">Authentik</span> (`https://authentik.example.com`) as an admin.
...
??? bug "**Troubleshooting**"
    ...
    - If login fails, review <span class="prod-app">Nextcloud</span>'s admin logs and <span class="prod-idp">Authentik</span>'s provider logs.
